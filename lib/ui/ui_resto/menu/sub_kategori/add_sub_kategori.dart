...
class AddSubKategoriActivity extends StatefulWidget {
  @override
  _AddSubKategoriActivityState createState() => _AddSubKategoriActivityState();
}

class _AddSubKategoriActivityState extends State<AddSubKategoriActivity> {
  TextEditingController _loginTextName = TextEditingController(text: "");
  TextEditingController _loginSubKatName = TextEditingController(text: "");
  ...
  Future<String?>? addSubKat() async{
    SharedPreferences pref = await SharedPreferences.getInstance();
    var token = pref.getString("token") ?? "";
    var id = pref.getInt("id") ?? "";

    var apiResult = await http.post(Uri.parse(Links.mainUrl + '/category'),
        body: {
          'name': _loginSubKatName.text,
        },
        headers: {
      ...
      child: Scaffold(
                ...
                Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      // ),
                      // SizedBox(height: CustomSize.sizeHeight(context) / 48,),
                      CustomText.bodyLight12(text: "Nama Sub Kategori", sizeNew: double.parse(((MediaQuery.of(context).size.width*0.03).toString().contains('.')==true)?(MediaQuery.of(context).size.width*0.03).toString().split('.')[0]:(MediaQuery.of(context).size.width*0.03).toString())),
                      SizedBox(
                        height: CustomSize.sizeHeight(context) * 0.005,
                      ),
                      TextField(
                        controller: _loginSubKatName,
        ...
        floatingActionButton: GestureDetector(
          onTap: () async{
            ...
            // pref.setString("email", _loginSubKatName.text.toString());
            ...
            if (_loginSubKatName.text != '') {
              addSubKat()!.whenComplete((){
                Navigator.pop(context);
