...
class DiskonOngkirActivity extends StatefulWidget {
  String tipeDiskonOngkir = '';

  DiskonOngkirActivity(this.tipeDiskonOngkir);

  @override
  _DiskonOngkirActivityState createState() => _DiskonOngkirActivityState(tipeDiskonOngkir);
}

class _DiskonOngkirActivityState extends State<DiskonOngkirActivity> {
  String tipeDiskonOngkir = '';

  _DiskonOngkirActivityState(this.tipeDiskonOngkir);
  ...
  Future<void> _getPromo(String lat, String long)async{
    ...
    var apiResult = await http.get(Uri.parse(Links.mainUrl + '/page/diskon-ongkir?lat=$lat&long=$long&i=$tipeDiskonOngkir'), headers: {
    ...
    page = (data['resto']['from'].toString() != 'null')?int.parse(data['resto']['from'].toString()):1;
    last_page = int.parse(data['resto']['last_page'].toString());

    if (data['resto']['data'].toString() != []) {
    ...
    if (data['msg'].toString() == 'success') {
      ...
      Navigator.pushReplacement(context,
          PageTransition(
              ...
              child: DiskonOngkirActivity(tipeDiskonOngkir)));
    }
  ...
  Future<void> _getPromo2(String lat, String long)async{
    ...
    SharedPreferences pref = await SharedPreferences.getInstance();
    String token = pref.getString("token") ?? "";
    var apiResult = await http.get(Uri.parse(Links.mainUrl + '/page/diskon-ongkir?page=$page&lat=$lat&long=$long&i=$tipeDiskonOngkir'), headers: {
      "Accept": "Application/json",
      "Authorization": "Bearer $token"
    });
    var data = json.decode(apiResult.body);
    print('next page');
    print(data.toString().contains('Penyetan Cok BG Junction'));
    print(data);
    print(data['resto']['from']);
    print(data['resto']['last_page']);
    ...
    for(var v in data['resto']['data']){
      ...
      print('contains');
      print(v['name'].toString().contains(promo.toString()));
      total = total + 1;
      print('total');
      print(total);
      print(total);
      if (v['name'].toString().contains(promo.toString()) == false) {
        promo.add(r);
      }
    }
          ...
          body: SafeArea(
            child: (isLoading)?Container(
                            ...
                            children: [
                              ...
                              Container(
                                ...
                                child: CustomText.textHeading3(
                                    text: (tipeDiskonOngkir == '0')?"Dapatkan Diskon ongkir 50% atau Potongan Ongkir Rp5.000 dengan minimum pembelanjaan Rp50.000 di merchant dibawah ini":"Dapatkan Free Ongkir atau Potongan Ongkir Rp10.000 dengan minimum pembelanjaan Rp100.000 di merchant dibawah ini",
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      Padding(
                                        ...
                                        child: CustomText.bodyRegular14(text: promo[index].distance.toString() + " Km", minSize: double.parse(((MediaQuery.of(context).size.width*0.03).toString().contains('.')==true)?(MediaQuery.of(context).size.width*0.03).toString().split('.')[0]:(MediaQuery.of(context).size.width*0.03).toString())),
                                      ),
                                      Padding(
                                        padding: EdgeInsets.only(left: CustomSize.sizeWidth(context) / 24),
                                        child: CustomText.bodyMedium16(text: promo[index].name, maxLines: 2, minSize: double.parse(((MediaQuery.of(context).size.width*0.04).toString().contains('.')==true)?(MediaQuery.of(context).size.width*0.04).toString().split('.')[0]:(MediaQuery.of(context).size.width*0.04).toString())),
                                      ),
                                    ],
                                  ),
                                ),
              ...
              children: [
                ...
                Container(child: CustomText.bodyMedium12(text: "kosong", minSize: double.parse(((MediaQuery.of(context).size.width*0.03).toString().contains('.')==true)?((MediaQuery.of(context).size.width*0.03)).toString().split('.')[0]:((MediaQuery.of(context).size.width*0.03)).toString())), alignment: Alignment.center, height: CustomSize.sizeHeight(context),),
              ],
            ),
          ),
